// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.sdwan.outputs;

import com.pulumi.core.annotations.CustomType;
import com.pulumi.sdwan.outputs.ServiceIpv6AclFeatureSequenceMatchEntryDestinationPort;
import com.pulumi.sdwan.outputs.ServiceIpv6AclFeatureSequenceMatchEntrySourcePort;
import java.lang.Integer;
import java.lang.String;
import java.util.List;
import java.util.Objects;
import java.util.Optional;
import javax.annotation.Nullable;

@CustomType
public final class ServiceIpv6AclFeatureSequenceMatchEntry {
    /**
     * @return Destination Data IP Prefix
     * 
     */
    private @Nullable String destinationDataPrefix;
    private @Nullable String destinationDataPrefixListId;
    /**
     * @return Destination Port List
     * 
     */
    private @Nullable List<ServiceIpv6AclFeatureSequenceMatchEntryDestinationPort> destinationPorts;
    /**
     * @return ICMP6 Message
     * 
     */
    private @Nullable List<String> icmpMessages;
    /**
     * @return next header number
     *   - Range: `0`-`255`
     * 
     */
    private @Nullable Integer nextHeader;
    /**
     * @return Packet Length
     * 
     */
    private @Nullable String packetLength;
    /**
     * @return Source Data IP Prefix
     * 
     */
    private @Nullable String sourceDataPrefix;
    private @Nullable String sourceDataPrefixListId;
    /**
     * @return Source Port List
     * 
     */
    private @Nullable List<ServiceIpv6AclFeatureSequenceMatchEntrySourcePort> sourcePorts;
    /**
     * @return TCP States
     *   - Choices: `syn`
     * 
     */
    private @Nullable String tcpState;
    /**
     * @return Select Traffic Class
     * 
     */
    private @Nullable List<Integer> trafficClasses;

    private ServiceIpv6AclFeatureSequenceMatchEntry() {}
    /**
     * @return Destination Data IP Prefix
     * 
     */
    public Optional<String> destinationDataPrefix() {
        return Optional.ofNullable(this.destinationDataPrefix);
    }
    public Optional<String> destinationDataPrefixListId() {
        return Optional.ofNullable(this.destinationDataPrefixListId);
    }
    /**
     * @return Destination Port List
     * 
     */
    public List<ServiceIpv6AclFeatureSequenceMatchEntryDestinationPort> destinationPorts() {
        return this.destinationPorts == null ? List.of() : this.destinationPorts;
    }
    /**
     * @return ICMP6 Message
     * 
     */
    public List<String> icmpMessages() {
        return this.icmpMessages == null ? List.of() : this.icmpMessages;
    }
    /**
     * @return next header number
     *   - Range: `0`-`255`
     * 
     */
    public Optional<Integer> nextHeader() {
        return Optional.ofNullable(this.nextHeader);
    }
    /**
     * @return Packet Length
     * 
     */
    public Optional<String> packetLength() {
        return Optional.ofNullable(this.packetLength);
    }
    /**
     * @return Source Data IP Prefix
     * 
     */
    public Optional<String> sourceDataPrefix() {
        return Optional.ofNullable(this.sourceDataPrefix);
    }
    public Optional<String> sourceDataPrefixListId() {
        return Optional.ofNullable(this.sourceDataPrefixListId);
    }
    /**
     * @return Source Port List
     * 
     */
    public List<ServiceIpv6AclFeatureSequenceMatchEntrySourcePort> sourcePorts() {
        return this.sourcePorts == null ? List.of() : this.sourcePorts;
    }
    /**
     * @return TCP States
     *   - Choices: `syn`
     * 
     */
    public Optional<String> tcpState() {
        return Optional.ofNullable(this.tcpState);
    }
    /**
     * @return Select Traffic Class
     * 
     */
    public List<Integer> trafficClasses() {
        return this.trafficClasses == null ? List.of() : this.trafficClasses;
    }

    public static Builder builder() {
        return new Builder();
    }

    public static Builder builder(ServiceIpv6AclFeatureSequenceMatchEntry defaults) {
        return new Builder(defaults);
    }
    @CustomType.Builder
    public static final class Builder {
        private @Nullable String destinationDataPrefix;
        private @Nullable String destinationDataPrefixListId;
        private @Nullable List<ServiceIpv6AclFeatureSequenceMatchEntryDestinationPort> destinationPorts;
        private @Nullable List<String> icmpMessages;
        private @Nullable Integer nextHeader;
        private @Nullable String packetLength;
        private @Nullable String sourceDataPrefix;
        private @Nullable String sourceDataPrefixListId;
        private @Nullable List<ServiceIpv6AclFeatureSequenceMatchEntrySourcePort> sourcePorts;
        private @Nullable String tcpState;
        private @Nullable List<Integer> trafficClasses;
        public Builder() {}
        public Builder(ServiceIpv6AclFeatureSequenceMatchEntry defaults) {
    	      Objects.requireNonNull(defaults);
    	      this.destinationDataPrefix = defaults.destinationDataPrefix;
    	      this.destinationDataPrefixListId = defaults.destinationDataPrefixListId;
    	      this.destinationPorts = defaults.destinationPorts;
    	      this.icmpMessages = defaults.icmpMessages;
    	      this.nextHeader = defaults.nextHeader;
    	      this.packetLength = defaults.packetLength;
    	      this.sourceDataPrefix = defaults.sourceDataPrefix;
    	      this.sourceDataPrefixListId = defaults.sourceDataPrefixListId;
    	      this.sourcePorts = defaults.sourcePorts;
    	      this.tcpState = defaults.tcpState;
    	      this.trafficClasses = defaults.trafficClasses;
        }

        @CustomType.Setter
        public Builder destinationDataPrefix(@Nullable String destinationDataPrefix) {

            this.destinationDataPrefix = destinationDataPrefix;
            return this;
        }
        @CustomType.Setter
        public Builder destinationDataPrefixListId(@Nullable String destinationDataPrefixListId) {

            this.destinationDataPrefixListId = destinationDataPrefixListId;
            return this;
        }
        @CustomType.Setter
        public Builder destinationPorts(@Nullable List<ServiceIpv6AclFeatureSequenceMatchEntryDestinationPort> destinationPorts) {

            this.destinationPorts = destinationPorts;
            return this;
        }
        public Builder destinationPorts(ServiceIpv6AclFeatureSequenceMatchEntryDestinationPort... destinationPorts) {
            return destinationPorts(List.of(destinationPorts));
        }
        @CustomType.Setter
        public Builder icmpMessages(@Nullable List<String> icmpMessages) {

            this.icmpMessages = icmpMessages;
            return this;
        }
        public Builder icmpMessages(String... icmpMessages) {
            return icmpMessages(List.of(icmpMessages));
        }
        @CustomType.Setter
        public Builder nextHeader(@Nullable Integer nextHeader) {

            this.nextHeader = nextHeader;
            return this;
        }
        @CustomType.Setter
        public Builder packetLength(@Nullable String packetLength) {

            this.packetLength = packetLength;
            return this;
        }
        @CustomType.Setter
        public Builder sourceDataPrefix(@Nullable String sourceDataPrefix) {

            this.sourceDataPrefix = sourceDataPrefix;
            return this;
        }
        @CustomType.Setter
        public Builder sourceDataPrefixListId(@Nullable String sourceDataPrefixListId) {

            this.sourceDataPrefixListId = sourceDataPrefixListId;
            return this;
        }
        @CustomType.Setter
        public Builder sourcePorts(@Nullable List<ServiceIpv6AclFeatureSequenceMatchEntrySourcePort> sourcePorts) {

            this.sourcePorts = sourcePorts;
            return this;
        }
        public Builder sourcePorts(ServiceIpv6AclFeatureSequenceMatchEntrySourcePort... sourcePorts) {
            return sourcePorts(List.of(sourcePorts));
        }
        @CustomType.Setter
        public Builder tcpState(@Nullable String tcpState) {

            this.tcpState = tcpState;
            return this;
        }
        @CustomType.Setter
        public Builder trafficClasses(@Nullable List<Integer> trafficClasses) {

            this.trafficClasses = trafficClasses;
            return this;
        }
        public Builder trafficClasses(Integer... trafficClasses) {
            return trafficClasses(List.of(trafficClasses));
        }
        public ServiceIpv6AclFeatureSequenceMatchEntry build() {
            final var _resultValue = new ServiceIpv6AclFeatureSequenceMatchEntry();
            _resultValue.destinationDataPrefix = destinationDataPrefix;
            _resultValue.destinationDataPrefixListId = destinationDataPrefixListId;
            _resultValue.destinationPorts = destinationPorts;
            _resultValue.icmpMessages = icmpMessages;
            _resultValue.nextHeader = nextHeader;
            _resultValue.packetLength = packetLength;
            _resultValue.sourceDataPrefix = sourceDataPrefix;
            _resultValue.sourceDataPrefixListId = sourceDataPrefixListId;
            _resultValue.sourcePorts = sourcePorts;
            _resultValue.tcpState = tcpState;
            _resultValue.trafficClasses = trafficClasses;
            return _resultValue;
        }
    }
}
